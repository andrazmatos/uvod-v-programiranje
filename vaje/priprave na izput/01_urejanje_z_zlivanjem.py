...
def v_katerem_je_manjsi(sez1, sez2):
    ...
    if not sez1:
        ...
        zmagovalec.append[sez1[0]]
        zmagovalec.append(v_katerem_je_manjsi(sez1[1:], sez2[1:]))
        return zmagovalec


# def v_katerem_je_manjsi(a, b):
#     return a if a and (b and a[0] < b[0] or not b) else b
...
def uredi(sez):
    if len(sez) <= 1:
        ...
        levi = uredi(sez[: len(sez) // 2])
        desni = uredi(sez[len(sez) // 2 :])
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDE5NywidXNlciI6MTE1Mzh9:1weHdk:Q8jD8pBph-N0lYPDQ1YikcZstO9tTSliMpeb6hpCyHE"
        try:
            tests = [
                ([1, 2, 3], [4, 5, 6], [1, 2, 3]),
                ([1], [2], [1]),
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDE5OCwidXNlciI6MTE1Mzh9:1weHdk:6A1JyqwCevrQqC4QTWMy1KW8-bAdSEzgMo32F5owPR0"
        try:
            tests = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDE5OSwidXNlciI6MTE1Mzh9:1weHdk:GENwN0PMT9K0zUGG9zbOXXMnraoUnGwC61dVs3XvDTs"
        try:
            tests = (
