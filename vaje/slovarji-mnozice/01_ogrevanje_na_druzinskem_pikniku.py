...
#     for oseba, sladica in njihovi.items():  ## generira pare
#         if oseba not in domaci.keys():
#             domaci[oseba] = sladica # njihovi[oseba]
#     return domaci
...
def nakupovalni_seznam(sladkosnedi, prisotni):
    sladice = {}
    for oseba in prisotni:
        if oseba in sladkosnedi:
            # if sladkosnedi[oseba] in sladice:
            #     sladice[sladkosnedi[oseba]] += 1
            # else:
            #     sladice[sladkosnedi[oseba]] = 1
            dessert = sladkosnedi[oseba]
            sladice[dessert] = sladice.get(dessert, 0) + 1
    return sladice

def get(slovar, kljuc, privzeta=None):
    if kljuc in slovar:
        return slovar[kljuc]
    else:
        return privzeta
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyMDMzNywidXNlciI6MTE1Mzh9:1w7EIL:gQmO8uzHLYDG7RCDR70Dj8OFTpKbniMH_TcLG5SxFhc"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyMDMzOCwidXNlciI6MTE1Mzh9:1w7EIL:XKGaqFXXwSl8RXm2tAA5fkQla6wggk8QsvxzzXbl-GY"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyMDMzOSwidXNlciI6MTE1Mzh9:1w7EIL:46jqSYjy-KMGCker5A42263bEDYizG58llmaoyLm880"
        try:
