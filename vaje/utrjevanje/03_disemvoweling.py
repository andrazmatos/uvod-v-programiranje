...
def disemvowel(sporocilo):
    ...
    for x in sporocilo:
        if x in 'aeiouAEIOU':
            ...
            niz_soglasnikov += x
    return niz_soglasnikov+niz_samoglasnikov
...
def razveljavi_disemvowel(niz):
    #prepišem vse samoglasnike iz konca
    niz_samoglasnikov = ''
    nov_niz = ''
    for n in niz:
        if n in 'aeiouAEIOU':
    ...
    for b in niz:
        if b in 'aeiouAEIOU':
            return nov_niz
        if b == '*':
            nov_niz += niz_samoglasnikov[0]
            niz_samoglasnikov = niz_samoglasnikov[1:]
        else:
            nov_niz += b
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNDIyNSwidXNlciI6MTE1Mzh9:1wCJ2q:4xuFuvw7sYRBehvi9zui00nMZ93Xn1RWAVXKFwxdd1w"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNDIyNiwidXNlciI6MTE1Mzh9:1wCJ2q:399yDp-zImcOOhhDXS6uIqphihsaAnZfhRdsEtYHfsw"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNDIyNywidXNlciI6MTE1Mzh9:1wCJ2q:JT6swlqx1cX8e-cvCJ4FES2XWkOAttchDz48cIRQVI4"
        try:
