...
def stevilo_clenov(m):
    if m <= 0:
        return 0
    for i in range(m+1):
        if vsota_produktov_sosednjih(i) > m:
            return i-1
...
def najblizje(a, b, m):
    ...
    for k in range(a, b+1):
        ...
        while vsota <= m:
            vsota += c * (c + 1)
            c += 1
        zgornji_kandidat = vsota - m 
        spodnji_kandidat = m - vsota + (c-1)*c
        if min(spodnji_kandidat, zgornji_kandidat) < min_razdalja:
            min_razdalja = min(spodnji_kandidat, zgornji_kandidat)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxNDc0LCJ1c2VyIjoxMTUzOH0:1wCJ2q:dkwP_0AAwyz45m9oXp-W7krvuqznkKqk44uUOX7oiSU"
        try:
            test_data = [
                ("""vsota_kvadratov(1)""", 1),
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxNDc1LCJ1c2VyIjoxMTUzOH0:1wCJ2q:Xi_yAuDjFlhiGA5i5mM2953iY2XtfVVkKh_02s06Vek"
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxNDc2LCJ1c2VyIjoxMTUzOH0:1wCJ2q:eqFR71fnfSXNwZl8m9I7RBedMyK8gjMbrTaI8x5NXiQ"
        try:
            test_data = [
                ("""stevilo_clenov(2)""", 1),
                ("""stevilo_clenov(19)""", 2),
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxNDc3LCJ1c2VyIjoxMTUzOH0:1wCJ2q:u94lBMm0MHLlHFJrSUsqvJ6zEH-Z-y2en726fo3xtWk"
        try:
            test_data = [
