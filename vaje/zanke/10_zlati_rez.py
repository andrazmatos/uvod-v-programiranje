...
def natancni_priblizek(eps):
    k = 1
    while abs(priblizek(k) - priblizek(k-1)) >= eps:
       k = k+1
    return priblizek(k)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMDAwLCJ1c2VyIjoxMTUzOH0:1w3DEL:8XTRK6fUXNKg2euDjfn_pRcJ-2CjHEc6cobKuCSLt9M"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMDAxLCJ1c2VyIjoxMTUzOH0:1w3DEL:BlbvXDHEI-WlJIJukJBo0H8OhElZ8WgJENjkIHtPxEc"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMDAyLCJ1c2VyIjoxMTUzOH0:1w3DEL:hgWMK4eFQNq9W8WwMgZglwpQFxJx_lr0UEs9dVxUzmM"
        try:
