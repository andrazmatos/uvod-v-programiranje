...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5NTAsInVzZXIiOjExNTM4fQ:1vx5wT:Ipf2QCIIITb6TWQrtc-20R94x-Wl1crfCI7IUxFyGQo"
        try:
            Check.equal('binomski_fakulteta(10, 2)', 45)
            Check.equal('binomski_fakulteta(8, 4)', 70)
            Check.equal('binomski_fakulteta(13, 9)', 715)
            if type(binomski_fakulteta(10, 2)) != int:
                Check.error('Funkcija binomski_fakulteta ne vrača celega števila.')
        except TimeoutError:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5NTEsInVzZXIiOjExNTM4fQ:1vx5wT:0Rc_-ynd_jwe8X5nrlsYKmD0PNAMUF1j-mgn-Ad_go8"
        try:
            Check.equal('binomski_rekurzija(10, 2)', 45)
            Check.equal('binomski_rekurzija(8, 4)', 70)
            Check.equal('binomski_rekurzija(13, 9)', 715)
            if type(binomski_rekurzija(10, 2)) != int:
                Check.error('Funkcija binomski_rekurzija ne vrača celega števila.')
        except TimeoutError:
            Check.error("Dovoljen čas izvajanja presežen")
        except Exception:
            Check.error(
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5NTIsInVzZXIiOjExNTM4fQ:1vx5wT:k6P4PioTCzFRZdIe-PaBImnjpykoWi18X-HJH2mW97Q"
        try:
