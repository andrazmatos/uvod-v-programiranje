...
def kolikokrat_se_pojavi_stevka(k,n):
    if n<10:
        ...
        enice = n%10
        if enice == k:
            return kolikokrat_se_pojavi_stevka(k, n//10) + 1
        else:
...
def ali_ima_enkratne_stevke(n):
    if n < 10:
        ...
        enice = n%10
        if kolikokrat_se_pojavi_stevka(enice, n) > 1:
            return False
        else:
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyMDAyNSwidXNlciI6MTE1Mzh9:1vx5wT:smbHhbXXDJRs5KYI6Ri2FZNSLkt_mfVrv-nasov-kiM"
        try:
            Check.equal('kolikokrat_se_pojavi_stevka(5, 294535)', 2)
            Check.equal('kolikokrat_se_pojavi_stevka(1, 1)', 1)
            Check.equal('kolikokrat_se_pojavi_stevka(0, 1)', 0)
            Check.equal('kolikokrat_se_pojavi_stevka(3, 33330)', 4)
            Check.equal('kolikokrat_se_pojavi_stevka(0, 10)', 1)
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyMDAyNiwidXNlciI6MTE1Mzh9:1vx5wT:uNxERZWctkBF0xCm4hXrzR1hSc74mSaQrpu3H15TH5I"
        try:
