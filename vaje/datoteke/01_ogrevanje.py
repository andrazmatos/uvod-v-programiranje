...
def groznje_z_dovolj_klicaji(datoteke):
    grozece = []
    for datoteka in datoteke:
        with open(datoteka, encoding='utf-8') as dat:
            if dat.read().count('!') >= 3:
                grozece.append(datoteka)
    return grozece
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyMDM0NiwidXNlciI6MTE1Mzh9:1wJuzt:ruFvfRe4xXjdkuzIZgVejr4fPP8QwaHe5SSj6V_ZP-w"
        try:
            test_cases = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyMDM0NywidXNlciI6MTE1Mzh9:1wJuzt:oQTmztSPCx7G5t_gYiEd1KUnWFMsEFlXGdQiYZKYnxQ"
        try:
            test_cases = [
