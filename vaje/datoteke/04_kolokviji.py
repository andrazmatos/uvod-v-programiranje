...
def nabor(niz):
    ...
    for i, vrednost in enumerate(vrednosti):
        if i>=1:
            vrednosti[i] = int(vrednost)
    return tuple(vrednosti)
...
def vsote(vhodna, izhodna):
    rezultati = nalozi_csv(vhodna)
    with open(izhodna, 'w', encoding='utf-8') as dat:
        for nabor in rezultati:
            print(nabor[0], sum(nabor[1:]) , file=dat, sep=',')
...
def rezultati(vhodna, izhodna):
    studenti = nalozi_csv(vhodna)
    def kljuc(nabor):
        return nabor[0].split(' ')[1]
    studenti.sort(key=kljuc)
    ...
    with open(izhodna, 'w', encoding='utf-8') as dat:
        for student in studenti:
            # print(student[0], *student[1:], sum(student[1:]), file=dat, sep=',')
            # * pomeni kot da nabor spremenim v vrsto, basically, da naštevam elemente po vrsti in se tako losam ()
            tocke = ''
            for tocka in student[1:]:
                tocke += str(tocka) +','
            print(f'{student[0]},{tocke}{sum(student[1:])}', file=dat)

            if skupne_tocke is None:
                skupne_tocke = list(student[1:])
                skupna_vsota = sum(student[1:])
            else:
                for i, tocke in enumerate(student[1:]):
                    skupne_tocke[i] += tocke
                skupna_vsota += sum(student[1:])

            st_studentov = len(studenti)
            tocke = ''
            for tocka in skupne_tocke:
                tocke += f'{tocka/st_studentov:.2f},'   ## :.2f pomeni zaokroženo na dve decimalki
        print(f'POVPRECEN STUDENT,{tocke}{skupna_vsota/st_studentov:.2f}', file=dat)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxNTE4LCJ1c2VyIjoxMTUzOH0:1wJuzt:FfOFu_m5c_rY_mkOMzjrkjObYDHSbHzG9PbG8dAoa70"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxNTE5LCJ1c2VyIjoxMTUzOH0:1wJuzt:4-Jlbb4UNpGT_U9BlSgXsws9sqRWfvETjoXf2zfMcyI"
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxNTIwLCJ1c2VyIjoxMTUzOH0:1wJuzt:enUrlEVczqVzV54B4bjdRH6Qyc1FXigmwxByB5QIdVU"
        try:
            test_data = [
                ("kolokviji_vhod.txt", ["Janez Novak,1,3,3,2,0", "Micka Kovaceva,0,3,2,3", "Miha Praznic", "Peter Klepec,1,0,1,2,1,3"],
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxNTIxLCJ1c2VyIjoxMTUzOH0:1wJuzt:VMYAAwkc4ToX_5azNzmgySHYcxckAq1mdUgSfPjbeiQ"
        try:
            test_data = [
