...
def koliko(niz, datoteka):
    with open(datoteka, 'w', encoding='utf-8') as dat:
        ...
        imena = niz.split(',')
        for ime in imena:
            if ime not in izpisana_imena:
                print(ime, kolikokrat_se_pojavi(niz, ime), file=dat)
                izpisana_imena.append(ime)
...
def koliko_iz_datoteke(vhodna, izhodna):
    with open(vhodna, 'r', encoding='utf-8') as d:
        niz_imen= ''
        for vrstica in d:
            niz_imen += vrstica
        niz_imen = niz_imen.strip().replace('\n',',')

        ### .strip() mi izbriše vse prazne znake
    return koliko(niz_imen, izhodna)
...
def koliko_urejen(vhodna, izhodna):
    with open(vhodna, 'r', encoding='utf-8') as d:
        ...
        for vrstica in d:
            niz_imen += vrstica
        niz_imen = niz_imen.strip().replace('\n',',')
    imena = niz_imen.split(',')
    pari = []
    for ime in imena:
        stevilo = kolikokrat_se_pojavi(niz_imen, ime)
        if (stevilo, ime) not in pari:
            pari.append((stevilo, ime))
    ...
    with open(izhodna, 'w', encoding='utf-8') as dat:
        for stevilo, ime in pari:
            if ime not in izpisana_imena:
                print(ime, stevilo, file=dat)
                izpisana_imena.append(ime)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxNTEwLCJ1c2VyIjoxMTUzOH0:1wJuzt:10ya2G_EpsD8wXmz6EkDjE_MDJ7UwEKd09-VB_SZdSY"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxNTExLCJ1c2VyIjoxMTUzOH0:1wJuzt:R1-QNFqpFQMsTzCY0PXS6Z145ezv2yUmtI4jcYwLpM4"
        try:
            test_cases = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxNTEyLCJ1c2VyIjoxMTUzOH0:1wJuzt:GDmLXcl1bNxVyyOdi20I6_-8QE1K9BTorLTIws8n6q4"
        try:
            test_cases = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxNTEzLCJ1c2VyIjoxMTUzOH0:1wJuzt:tpEBdJmogGyAs8s77FssabS7Ql7me9m134a72WjhWlg"
        try:
            test_cases = [
