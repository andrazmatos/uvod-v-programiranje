...
def preberi_celo_stevilo():
    vr = input("> Vnesi celo število: ")
    if vr.isnumeric():
        return int(vr)
    elif vr[0] == '-' and vr[1:].isnumeric():
        return int(vr)
    else:
        print(f'Žal "{vr}" ni celo število, poskusi ponovno!')
...
def ugibaj(resitev):
    ...
    if ugib == resitev:
        print(f"BRAVO! Res sem si zamislil število {resitev}!")
    elif ugib > resitev:
        print('Moje število je manjše!')
        return ugibaj(resitev)
    else:
...
def racunalnik_ugiba(spodnja,zgornja):
    if spodnja == zgornja:
        print(f'Juhu, uganil sem! Zamislil si si število {spodnja}!')
        return
    sredina=(spodnja+zgornja)//2
    print(f'Ali je tvoje število Enako/Večje/Manjše od {sredina}?')
    odgovor = input('E/V/M> ')
    if odgovor.lower()=='e':
        print(f'Juhu, uganil sem! Zamislil si si število {sredina}!')
    elif odgovor.lower() == 'v':
        racunalnik_ugiba(sredina + 1, zgornja)
    elif odgovor.lower() == 'm':
        racunalnik_ugiba(spodnja,sredina - 1)
    else:
        print("Ali lahko daš normalen odgovor?")
        racunalnik_ugiba(spodnja, zgornja)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNzQ5NiwidXNlciI6MTE1Mzh9:1vzdCS:X1s8Jn3cyx8mhvoE2rbjorJPB4ku2ENmeCvabU_3YpI"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNzQ5NywidXNlciI6MTE1Mzh9:1vzdCS:r3vGRLJzggEPv79SxgGJzMZ4eSUD8-yssp79QnczfTU"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNzYyMywidXNlciI6MTE1Mzh9:1vzdCS:Dzn33JQDSk8-pxwBWc7OU2UgGm69XhjLDRHsto_pKz4"
        try:
