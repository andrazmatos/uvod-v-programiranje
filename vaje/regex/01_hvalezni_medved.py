...
def dvojne_crke(niz):
    vzorec = r'(\w*(\w)\2\w*)'
    pari = set(re.findall(vzorec, niz)) #vrne obe skupini
    return {beseda for beseda, crka in pari}
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozMzM5NywidXNlciI6MTE1Mzh9:1wOzpJ:JLZOYbyzIsmfkphdCUMuCVS3UYy9nCUfZ1JSIBmpQ3E"
        try:
            Check.equal(
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozMzM5OSwidXNlciI6MTE1Mzh9:1wOzpJ:O01kLxm3BTckvpIP1akasp8-XryEH8dIEq9X53mBnpk"
        try:
            hvalezni_medved = """Gori nekje v gorah, ne ve se več, ali je bilo pri Macigoju ali
            Naravniku, je šivala gospodinja v senci pod drevesom in zibala otroka. Naenkrat
            prilomasti - pa prej ni ničesar opazila - medved in ji moli taco, v kateri je
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozMzQwMSwidXNlciI6MTE1Mzh9:1wOzpJ:R9JbD4yQc4wCpGRFbYRXBGGdmiEMpPsvwBpVG_guZoo"
        try:
            hvalezni_medved = """Gori nekje v gorah, ne ve se več, ali je bilo pri Macigoju ali
            Naravniku, je šivala gospodinja v senci pod drevesom in zibala otroka. Naenkrat
            prilomasti - pa prej ni ničesar opazila - medved in ji moli taco, v kateri je
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozMzQwMCwidXNlciI6MTE1Mzh9:1wOzpJ:nPezsabVCR7SGcqaCpo5E1V1Qr77PY3UHVpQzhvwiuE"
        try:
            hvalezni_medved = """Gori nekje v gorah, ne ve se več, ali je bilo pri Macigoju ali
            Naravniku, je šivala gospodinja v senci pod drevesom in zibala otroka. Naenkrat
            prilomasti - pa prej ni ničesar opazila - medved in ji moli taco, v kateri je
