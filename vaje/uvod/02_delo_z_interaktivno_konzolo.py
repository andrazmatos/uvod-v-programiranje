...
# =====================================================================@000913=
# 4. podnaloga
# Vpišite izraz, katerega vrednost je enaka $1 + 2 + \dots + 2025$.
# =============================================================================
2025*2026//2
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MTAsInVzZXIiOjExNTM4fQ:1vuYgW:L69MH6jjVQnmkv_BPOBMcES_xxCBJmjiRUpEw2WIK0o"
        try:
            try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MTEsInVzZXIiOjExNTM4fQ:1vuYgW:UWKr2yC13ZRh8a_ij32Gq9pBt67IIivWxJ1zt8ujz10"
        try:
            try:
                rezultat = float(Check.current_part['solution'])
                if 0 < rezultat < 1:
                    for i in range(1, 11):
                        if round(rezultat, i) != round(0.4107812905029088, i):
                            Check.error('Število je natančno le na {} decimalk.'.format(i))
                            break
                else:
                    Check.error('Rezultat bi moral ležati med 0 in 1.')
            except:
                Check.error('Napisati morate število s plavajočo vejico.')
        except TimeoutError:
            Check.error("Dovoljen čas izvajanja presežen")
        except Exception:
            Check.error(
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MTIsInVzZXIiOjExNTM4fQ:1vuYgW:BS_NfG2uslQsmyA0Sf-CX-xbPFVC2XmYadP-lc495NI"
        try:
            try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MTMsInVzZXIiOjExNTM4fQ:1vuYgW:PukmYOh4FgF6_KoEWGI5U7smKuv_qId9SezTm6BIA2A"
        try:
            try:
